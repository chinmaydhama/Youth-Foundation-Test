...
with tab1:
    ...
    with st.expander("Show/Hide Raw Weekly Teachers Data"):
        st.dataframe(
            weekly_teachers.style.format({"teachers_registered": "{:,.0f}"})
        )
    ...
    numeric_cols_pivot = pivot_df.select_dtypes(include=[np.number]).columns
    with st.expander("Show/Hide Pivot Table"):
        st.dataframe(
            pivot_df.style.format("{:,.0f}", subset=numeric_cols_pivot)
        )
with tab2:
    ...
    with st.expander("Show/Hide Weekly Assessments Data"):
        st.dataframe(
            weekly_assessments.style.format({
                "baseline_assessments": "{:,.0f}",
                "endline_assessments": "{:,.0f}"
            })
        )
...
with tab3:
    ...
    with st.expander("Show/Hide Weekly Learning Level Data"):
        st.dataframe(
            weekly_levels.style.format({
                "baseline_average_level": "{:.2f}",
                "endline_average_level": "{:.2f}"
            })
        )

    st.markdown("---")

    st.subheader("Comparison: Baseline vs. Endline by District/Block/School")
...
with tab4:
    ...
    with st.expander("Show/Hide Weekly Tutoring Calls Data"):
        st.dataframe(
            weekly_tutoring_calls.style.format({"tutoring_calls": "{:,.0f}"})
        )
    ...
    with st.expander("Show/Hide Top 10 Schools Data"):
        st.dataframe(
            top_schools_tutoring.style.format({"tutoring_calls": "{:,.0f}"})
        )
...
with tab6:
    ...
    if not np.issubdtype(filtered_df[pivot_values].dtype, np.number):
        ...
        with st.expander("Show/Hide Pivot Table"):
            ...
            numeric_cols_pivot_data = pivot_data.select_dtypes(include=[np.number]).columns
            st.dataframe(
                pivot_data.style.format("{:,.2f}", subset=numeric_cols_pivot_data)
            )
